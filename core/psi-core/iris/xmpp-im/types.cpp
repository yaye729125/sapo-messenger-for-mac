...
bool Message::isSapoSMS() const
{
	return d->isSapoSMS;
}

void Message::setSapoSMSProperties(const QVariantMap &props)
{
	d->sapoSMSProps = props;
	
	if (!props.isEmpty())
		d->isSapoSMS = true;
}

const QVariantMap & Message::sapoSMSProperties() const
{
...
Stanza Message::toStanza(Stream *stream) const
{
	...
	// Sapo:SMS
	if (!d->sapoSMSProps.isEmpty()) {
		QDomElement smsElem = s.createElement("sapo:sms", "x");
		foreach(QString prop, d->sapoSMSProps.keys()) {
			QDomElement textElem = s.createTextElement("sapo:sms", prop, d->sapoSMSProps[prop].toString());
			smsElem.appendChild(textElem);
		}
...
bool Message::fromStanza(const Stanza &s, int timeZoneOffset)
{
	...
	// Sapo:SMS
	QDomElement x = root.elementsByTagNameNS("sapo:sms", "x").item(0).toElement();
	if (!x.isNull()) {
		d->isSapoSMS = true;

		for(QDomNode node = x.firstChild(); !node.isNull(); node = node.nextSibling()) {
			QDomElement nodeElem = node.toElement();
			if(nodeElem.isNull())
				continue;
			
			d->sapoSMSProps[nodeElem.tagName()] = ( nodeElem.hasChildNodes() ?
...
const QString& Status::sapoPhotoHash() const
{
	return v_sapoPhotoHash;
}

void Status::setSapoPhotoHash(const QString& h)
{
	v_sapoPhotoHash = h;
	v_hasSapoPhotoHash = true;
}

bool Status::hasSapoPhotoHash() const
{
	return v_hasSapoPhotoHash;
}
