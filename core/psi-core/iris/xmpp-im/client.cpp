...
public:
	...
	QSet<QString> rosterSubsyncAllowedDomains;
};
...
:QObject(par)
{
	...
	// roster-subsync
	d->features.addFeature("http://delx.cjb.net/protocol/roster-subsync");
}
...
void Client::ppRosterSubsync(const Jid &jid, const QString &nick, const Roster &roster)
{
	if (rosterSubsyncAllowedDomainsSet().contains(jid.domain())) {
		importRoster(roster);
		
		// Automatically subscribed
		JT_Presence *presSub = new JT_Presence(rootTask());
		presSub->sub(jid, "subscribed", nick);
		presSub->go(true);
		
		foreach (RosterItem item, roster) {
			JT_Roster *r = new JT_Roster(rootTask());
			r->set(jid, jid.bare(), item.groups());
			r->go(true);
			
			JT_Presence *presSub = new JT_Presence(rootTask());
			presSub->sub(jid, "subscribe", "");
			presSub->go(true);
		}
	}
}
...
void Client::addRosterSubsyncAllowedDomain(const QString &domain)
{
	d->rosterSubsyncAllowedDomains += domain;
}

void Client::clearRosterSubsyncAllowedDomainsSet()
{
	d->rosterSubsyncAllowedDomains.clear();
}

const QSet<QString> & Client::rosterSubsyncAllowedDomainsSet()
{
	return d->rosterSubsyncAllowedDomains;
}
