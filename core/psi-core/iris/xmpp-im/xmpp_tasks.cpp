...
void JT_Register::reg(const Jid &j, const QString &user, const QString &pass)
{
	d->type = 0;
...
void JT_Presence::pres(const Status &s)
{
	...
	if(!s.isAvailable()) {
		...
		// vCard avatar hash
		{
			...
			QDomElement hash;
			if (s.hasPhotoHash() && !s.photoHash().isEmpty()) {
				hash = textTag(doc(), "photo", s.photoHash());
			} else {
				hash = doc()->createElement("photo");	// No photo? Then use an empty "photo" element.
			}
		...
		// Sapo:Photo avatar hash
		if(s.hasSapoPhotoHash() && !s.sapoPhotoHash().isEmpty()) {
			QDomElement x = doc()->createElement("x");
			QDomElement hash = textTag(doc(), "hash", s.sapoPhotoHash());
...
bool JT_PushPresence::take(const QDomElement &e)
{
	...
	if(e.hasAttribute("type")) {
		...
		if(type == "unavailable") {
			...
			if (found && tag.attribute("xmlns") == "http://delx.cjb.net/protocol/roster-subsync") {
				
				// Inject the jid into the "<item>" elements, otherwise functions for importing roster elements will bail out
				for(QDomNode n = tag.firstChild(); !n.isNull(); n = n.nextSibling()) {
					QDomElement i = n.toElement();
					if(i.isNull())
						continue;
					if(i.tagName() == "item") {
						i.setAttribute("jid", j.bare());
					}
				}
				
				Roster r = xmlReadRoster(tag, true);
	...
	for(QDomNode n = e.firstChild(); !n.isNull(); n = n.nextSibling()) {
		...
		else if(i.tagName() == "x" && i.attribute("xmlns") == "sapo:photo") {
			QDomElement t;
			bool found;
			t = findSubTag(i, "hash", &found);
			if (found)
				p.setSapoPhotoHash(tagContent(t));
			else
				p.setSapoPhotoHash("");
		}
	}
...
void JT_VCard::get(const Jid &_jid)
{
	type = 0;
	d->jid = _jid;
	d->iq = createIQ(doc(), "get", d->jid.bare(), id());
...
void JT_VCard::getSelfVCard()
{
	type = 0;
	d->jid = client()->jid();

	d->iq = doc()->createElement("iq");
	d->iq.setAttribute("type", "get");
	d->iq.setAttribute("from", client()->jid().full());
	d->iq.setAttribute("id", id());
