...
void AdvancedConnector::setDefaultFallbackHostPort(const QString &host, Q_UINT16 _port)
{
	if(d->mode != Idle)
		return;
	d->default_host = host;
	d->default_port = _port;
}
...
#endif
		d->using_srv = false;
		if (!d->default_host.isEmpty()) {
			d->host = d->default_host;
			d->port = d->default_port;
		}
		else {
